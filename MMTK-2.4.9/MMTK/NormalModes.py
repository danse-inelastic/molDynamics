...
if dsyevd:
    ...
    ev = Numeric.zeros((n,), Numeric.Float)
...
class NormalModes:
	...
	if dsyevd is None:
		...
		modes = modes.real
            ...
            iwork = Numeric.zeros((liwork,), int_type)
            results = dsyevd('V', 'L', self.nmodes, self.array, self.nmodes,
...
class SubspaceNormalModes(NormalModes):
	...
	if type(self.basis) is type(()):
        ...
        if nexcluded > 0:
            ...
            vt = Numeric.zeros((nexcluded, min_n_m), Numeric.Float)
            work = Numeric.zeros((1,), Numeric.Float)
            ...
            u = Numeric.zeros((min_n_m, 3*natoms), Numeric.Float)
            vt = Numeric.zeros((ntotal, min_n_m), Numeric.Float)
            work = Numeric.zeros((1,), Numeric.Float)
            ...
            u = Numeric.zeros((min_n_m, 3*natoms), Numeric.Float)
            vt = Numeric.zeros((nmodes, min_n_m), Numeric.Float)
            work = Numeric.zeros((1,), Numeric.Float)
            iwork = Numeric.zeros((8*min_n_m,), int_type)
